...
MyApp::StartApplication (void)
{
  ...
  m_socket->SetRecvCallback(MakeCallback(&MyApp::Recieve, this));
  SendPacket ();

}

void MyApp::Recieve(Ptr<Socket> socket){
  NS_LOG_FUNCTION(this);

  NS_LOG_INFO("RECIEVED");
}
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("logging", "Logging", logging);
  cmd.Parse (argc, argv);

  if(logging){
    LogComponentEnable("Swift", LOG_LEVEL_INFO);
    LogComponentEnable("TcpSwift", LOG_LEVEL_INFO);
  }

  NodeContainer nodes;
  nodes.Create (2);

  Config::SetDefault("ns3::TcpL4Protocol::SocketType", TypeIdValue(TypeId::LookupByName("ns3::TcpSwift")));
  Config::SetDefault("ns3::TcpSocket::SegmentSize", UintegerValue(8));
